...
lookUp={'C':0,'C#':1,'D':2,'D#':3,'E':4,'F':5,'F#':6,'G':7,'G#':8,'A':9,'A#':10,'B':11,0:0.25,1:0.5,2:1,3:2,4:4}#R
...
def loadMusic(f):
    ...
    for i in range(len(ns)):
        ns[i]=ns[i].split(',')
    if(ns[0][0]=='R'):
        possibleStarts.append(128+int(ns[0][1]))
    else:
        possibleStarts.append(lookUp[ns[0][0]]+int(ns[0][1])*12)
    for i in range(len(ns)-1):
        if(ns[i][0]=='R' and ns[i+1][0]=='R'):
            ajm[128+int(ns[i][1])].addNextRest(int(ns[i+1][1]))
        elif(ns[i][0]=='R'):
            ajm[128+int(ns[i][1])].addNextNote(lookUp[ns[i+1][0]]+int(ns[i+1][1])*12)
        elif(ns[i+1][0]=='R'):
            ajm[lookUp[ns[i][0]]+int(ns[i][1])*12].addNextRest(int(ns[i+1][1]))
        else:
            ajm[lookUp[ns[i][0]]+int(ns[i][1])*12].addNextNote(lookUp[ns[i+1][0]]+int(ns[i+1][1])*12)
...
restShift=0
time=0
while time*0.25+restShift<tempo*duration:
    print(time,time*0.25+restShift,currentNote)
    if(currentNote>127):
        restShift+=lookUp[nextNote-128]
        time+=int((lookUp[nextNote-128])/0.25)
    else:
        mf.addNote(0,0,currentNote,time*0.25+restShift,0.25,volume)
        time+=1
